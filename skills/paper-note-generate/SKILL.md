...
description: Maintain Research-Flow's paper `note.md` generation prompt, managed note blocks, figure/table evidence rendering, and tests. Use when changing structured paper summaries, visual evidence in notes, paper note schemas, or prompt rules for deep academic paper reading.
---
...
3. Pass section text plus compact figure/table evidence to `paper_note_generate.default`.
4. Require JSON-only blocks from the LLM.
5. Render managed note blocks deterministically.
6. Always inject available image Markdown into prompt-defined note sections: method/problem figures go under `method`, result/appendix figures go under `experimental_results`; do not create a separate top-level visual block.
...
- Generated notes must be detailed deep-reading notes, not short summaries. Follow the prompt's minimum length requirements unless supplied section evidence is genuinely missing.
...
- Prompt: `backend/config/prompts/paper_note_generate.md`
