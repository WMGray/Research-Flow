...
SECTION_FILES: tuple[tuple[str, str, str, int], ...] = (
    ("method", "Method", "03_method.md", 2800),
    ("introduction", "Introduction", "01_introduction.md", 2400),
    ("experiment", "Experiment", "04_experiment.md", 1800),
    ("related_work", "Related Work", "02_related_work.md", 1800),
    ("conclusion", "Conclusion", "05_conclusion.md", 1200),
)
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    note_path = Path(args.note_path) if args.note_path else sources_root / "03-paper-note-generate" / case / "note.md"
    source_dir = Path(args.source_dir) if args.source_dir else sources_root / "04-paper-knowledge-mining" / case
    ...
    note_context = _compact(_read_text(note_path), args.note_max_chars)
    ...
    knowledge_path = run_dir / "knowledge.json"
    knowledge_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    (run_dir / "metadata.json").write_text(metadata_json, encoding="utf-8")
    (run_dir / "section_context.md").write_text(section_context, encoding="utf-8")
    (run_dir / "note_context.md").write_text(note_context, encoding="utf-8")
    ...
    summary = {
        ...
        "note_context_char_count": len(note_context),
        "llm_called": True,
    }
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Run paper-knowledge-mining on skill-lab sources.")
    ...
    parser.add_argument("--output-root", default=str(REPO_ROOT / "skill-lab" / "runs" / "paper-knowledge-mining"))
    parser.add_argument("--output-dir")
    parser.add_argument("--instruction-key", default="paper_knowledge_mining.default")
    parser.add_argument("--feature", default="default_chat")
    parser.add_argument("--max-tokens", type=int, default=6000)
    parser.add_argument("--note-max-chars", type=int, default=6000)
