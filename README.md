...
Research-Flow 是面向科研活动的管理平台，用于统一管理论文、课题、组会汇报、数据集与 Knowledge 等科研资源，并通过自动化辅助与人工审核相结合，降低科研管理成本，提升研究效率。
...
平台围绕 `Paper`、`Project`、`Presentation`、`Dataset`、`Knowledge` 五类核心资源组织能力，上述路径用于描述典型使用场景。
...
- **Zotero 集成** — 通过 `Zotero MCP` 读取现有文献库与 Collection，支持导入与关联
- **Knowledge 管理** — 从文献中提取观点、定义与特征等结构化知识，形成可检索、可关联的 Knowledge 资源
- **课题管理** — 以 Overview / Related Work / Method / Experiment / Conclusion / Manuscript 六模块管理课题内容、关联材料与阶段产出
- **实验规划** — 汇总当前课题相关论文的实验设置，结合课题中的 Related Work 生成实验建议
- **PPT 制作** — 基于课题与论文材料生成组会汇报大纲、逐页草稿与可导出的 PPT
- **论文写作** — 基于课题积累生成并迭代完整论文草稿，支持逐章节编辑
- **投稿 & Rebuttal** — 格式检查、审稿意见逐条回复辅助
- **会议追踪** — 集中管理投稿截止、审稿通知等关键时间节点
- **知识图谱** — 论文、数据集、课题与 Knowledge 资源之间的关系可视化
